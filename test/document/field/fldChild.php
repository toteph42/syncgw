...
class fldChild extends \syncgw\document\field\fldChild {
	...
	public function testClass(string $typ, float $ver, string $xpath): void {
		...
		if ($typ == 'application/activesync.contact+xml') {
			...
			$ext->loadXML('<syncgw><ApplicationData><Body><Type>3</Type><EstimatedDataSize>5500</EstimatedDataSize>'.
						...
						'<Children><Child>Anna</Child><Child>Bob</Child><Child>Justin</Child></Children>'.
			...
			$cmp1 = '<Data><'.self::TAG.'>Anna</'.self::TAG.'><'.self::TAG.'>Bob</'.self::TAG.'>'.
					'<'.self::TAG.'>Justin</'.self::TAG.'></Data>';
			$cmp2 = new XML();
			$cmp2->loadXML('<Data><'.$xpath.' xml-ns="activesync:Contacts"><'.self::TAG.'>Anna</'.self::TAG.'><'.
						self::TAG.'>Bob</'.self::TAG.'><'.self::TAG.'>Justin</'.self::TAG.'></'.$xpath.'></Data>');
		}
