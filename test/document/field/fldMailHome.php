...
class fldMailHome extends \syncgw\document\field\fldMailHome {
	 ...
	 *  @return - Class object
	 */
	public static function getInstance(): fldMailHome {
	...
	public function testClass(string $typ, float $ver, string $xpath): void {
		...
		if ($typ == 'application/activesync.gal+xml') {
			$ext = new XML();
			$ext->loadXML('<syncgw><ApplicationData><EmailAddress>"Anat Kerry (anat@contoso.com)"&lt;anat@contoso.com&gt;</EmailAddress>'.
					'</ApplicationData></syncgw>');
			$cmp1 = '<Data><'.self::TAG.'>"Anat Kerry (anat@contoso.com)"&lt;anat@contoso.com&gt;</'.self::TAG.'></Data>';
			$cmp2 = new XML();
			$cmp2->loadXML('<Data><'.$xpath.' xml-ns="'.XML::CP[XML::AS_GAL].'">"Anat Kerry (anat@contoso.com)"&lt;anat@contoso.com&gt;</'.
					$xpath.'></Data>');
		}
		...
		if ($typ == 'application/activesync.contact+xml') {
			...
			$cmp1 = '<Data><'.self::TAG.'>"Anat Kerry (anat@contoso.com)"&lt;anat@contoso.com&gt;</'.self::TAG.'></Data>';
			$cmp2 = new XML();
			$cmp2->loadXML('<Data><'.$xpath.' xml-ns="'.XML::CP[XML::AS_CONTACT].'">"Anat Kerry (anat@contoso.com)"&lt;anat@contoso.com&gt;</'.
					$xpath.'></Data>');
		}
