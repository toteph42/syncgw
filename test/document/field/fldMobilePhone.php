...
class fldMobilePhone extends \syncgw\document\field\fldMobilePhone {
	 ...
	 *  @return - Class object
	 */
	public static function getInstance(): fldMobilePhone {
	...
	public function testClass(string $typ, float $ver, string $xpath): void {
		...
		if ($typ == 'application/activesync.gal+xml') {
			$ext = new XML();
			$ext->loadXML('<syncgw><ApplicationData><MobilePhone>(206) 555-0102</MobilePhone></ApplicationData></syncgw>');
			$cmp1 = '<Data><'.self::TAG.'>(206) 555-0102</'.self::TAG.'></Data>';
			$cmp2 = new XML();
			$cmp2->loadXML('<Data><'.$xpath.' xml-ns="'.XML::CP[XML::AS_GAL].'">(206) 555-0102</'.$xpath.'></Data>');
		}
		...
		if ($typ == 'application/activesync.contact+xml') {
			...
			$cmp1 = '<Data><'.self::TAG.'>(206) 555-0102</'.self::TAG.'></Data>';
			$cmp2 = new XML();
			$cmp2->loadXML('<Data><'.$xpath.' xml-ns="'.XML::CP[XML::AS_CONTACT].'">(206) 555-0102</'.$xpath.'></Data>');
		}
