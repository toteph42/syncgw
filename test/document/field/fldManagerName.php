...
class fldManagerName extends \syncgw\document\field\fldManagerName {
	 ...
	 *  @return - Class object
	 */
	public static function getInstance(): fldManagerName {
	...
	public function testClass(string $typ, float $ver, string $xpath): void {
		...
		if ($typ == 'application/activesync.contact+xml') {
			...
			$ext->loadXML('<syncgw><ApplicationData><Body><Type>3</Type><EstimatedDataSize>5500</EstimatedDataSize>'.
					...
					'<ManagerName>Manager name</ManagerName><Picture>/9j/4AAQSkZJRgABAQEAYABgAAD/...</Picture>'.
					'<NativeBodyType>3</NativeBodyType></ApplicationData></syncgw>');
			$cmp1 = '<Data><'.self::TAG.'>Manager name</'.self::TAG.'></Data>';
			$cmp2 = new XML();
			$cmp2->loadXML('<Data><'.$xpath.' xml-ns="activesync:Contacts2">Manager name</'.$xpath.'></Data>');
		}
