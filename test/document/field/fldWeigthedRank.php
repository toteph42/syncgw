...
class fldWeigthedRank extends \syncgw\document\field\fldWeigthedRank {
	...
	public function testClass(string $typ, float $ver, string $xpath): void {
		...
		if ($typ == 'application/activesync.contact+xml') {
			...
			$ext->loadXML('<syncgw><ApplicationData><Type>3</Type><EstimatedDataSize>5500</EstimatedDataSize>'.
					'<WeigthedRank>0</WeigthedRank><NameMiddle>M.</NameMiddle>'.
					'<NativeBodyType>3</NativeBodyType></ApplicationData></syncgw>');
			$cmp1 = '<Data><'.self::TAG.'>0</'.self::TAG.'></Data>';
			$cmp2 = new XML();
			$cmp2->loadXML('<Data><'.$xpath.' xml-ns="activesync:Contacts">0</'.$xpath.'></Data>');
		}
