   ...
   /**
	...
	public function getInfo(XML &$xml, bool $status): void {
		...
		$xml->addVar('Opt', '<a href="https://learn.microsoft.com/en-us/openspecs/exchange_server_protocols/ms-ascmd" target="_blank">[MS-ASCMD]</a> '.
				      sprintf(_('Exchange ActiveSync &lt;%s&gt; handler'), 'SmartForward'));
	...
	public function Parse(XML &$in, XML &$out): bool {
		...
		Debug::Msg($in, '<SmartForward> input'); //3
		...
		if (!$rc) {
			...
			if (!$db->SendMail(FALSE, $doc)) {
				Debug::Msg('<SmartForward> failed'); //3
				$rc = masStatus::SERVER;
			} else
				$rc = masStatus::SUBMIT;
		}
		...
		$out->getVar('SmartForward'); //3
		Debug::Msg($out, '<SmartForward> output'); //3
