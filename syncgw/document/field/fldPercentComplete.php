...
class fldPercentComplete extends fldHandler {

	// module version number
	const VER = 6;

	// ----------------------------------------------------------------------------------------------------------------------------------------------------------
	const TAG 				= 'PercentComplete';
	...
	public function import(string $typ, float $ver, string $xpath, $ext, string $ipath, XML &$int): bool {
		...
		case 'text/x-vcalendar':
			foreach ($ext as $rec) {
				...
				// check type
				if (!is_numeric($rec['D'])) {
					Debug::Msg('['.$rec['T'].'] ['.$rec['D'].'] not "integer" - dropping record'); //3
					continue;
				}
				if ($rec['D'] < 0 || $rec['D'] > 100) {
					Debug::Msg('['.$rec['T'].'] value "'.$rec['D'].'" not between "0-100" - dropping record'); //3
					continue;
				}
				// check parameter
