...
class fldKind extends fldHandler {
		...
		case 'text/x-vcard':
			foreach ($ext as $rec) {
				if ($rec['T'] != $xpath)
					continue;
				// check type
				$rec['D'] = strtolower($rec['D']);
				$s = 'individual group org location x-';
				if (strpos($s, $rec['D']) === FALSE && substr($rec['D'], 0, 2) != 'x-') {
					Debug::Msg('['.$rec['T'].'] ['.$rec['D'].'] constant mismatch "'.$s.'" - dropping record'); //3
					continue;
				}
				// check parameter
